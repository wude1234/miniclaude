#!/usr/bin/env python3
"""
v5_interview_agent.py - Mini Claude Code: interview-enhanced agent.
...
- quote normalization and uniqueness checks for edit_file
- unified diff feedback
- permission modes: plan/default/acceptEdits/bypassPermissions/dontAsk
- larger tool set: list_files, grep_search, run_shell, memory, state
- lightweight context compression for large/stale tool results
- subagent token aggregation
- improved SKILL.md metadata parsing
...
class Agent:
    ...
    def default_system(self) -> str:
        return f"""You are Mini Claude Code v5 at {self.workdir}.
...
- After finishing, summarize changes and mention any limits."""
    ...
    def compress_messages(self) -> Optional[str]:
        ...
        if size_before < SOFT_CONTEXT_CHARS:
            return None
    ...
    def run_once(self, prompt: str, max_turns: int = 40) -> Dict[str, Any]:
        ...
        return {"text": final_text, "tokens": self.usage}
    ...
    def execute_tool(self, name: str, args: Dict[str, Any]) -> str:
        ...
        try:
            ...
            if name == "recall_memory":
                return self.memory.recall(str(args["query"]), int(args.get("limit", 5)))
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Mini Claude Code v5 interview-enhanced agent")
...
def main() -> None:
    ...
    print(f"Mini Claude Code v5 - {WORKDIR}")
