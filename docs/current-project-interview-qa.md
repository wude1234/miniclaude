...
这一节不是普通项目总结，而是按大厂 Agent / AI 研发面试官的判断方式来准备。核心目标是让面试官相信：你不是只会调 API，而是理解 Coding Agent Runtime 的关键工程问题。

### 一、专家结论

这个项目适合放在 AI 研发简历里，但表述重点必须从“实现了很多工具”改成“复现并扩展了一个轻量级 Coding Agent Runtime”。真正有价值的是 Agent 闭环、上下文治理、工具协议、安全编辑和外部工具生态，而不是文件读写、命令执行这些单点功能。

从专业角度看，它可以被定位为：

> 基于 Python 复现并扩展 Claude Code 类 Coding Agent 的核心运行时，围绕 ReAct Agent Loop、Tool Calling、Observation 回写、上下文压缩、文件安全、Sub-agent/Skills 和 MCP 工具生态，构建可运行、可测试、可解释的面试版 Agent 系统。

这个定位比“从零实现 Claude Code”更稳，因为它既能体现工程深度，也不会被 GitHub 历史和生产级能力追问打穿。

### 二、面试官评分视角

如果我是 Agent 算法 / AI 研发面试官，会从四个维度判断这个项目。

第一，看你是否理解 Agent 和普通 LLM 应用的区别。高分回答要落在闭环：模型决策、Tool Calling、工具执行、Observation 回写、继续推理。不要只说“接了 LLM API”。

第二，看你是否理解长任务 Agent 的工程风险。高分回答要讲上下文膨胀、工具结果污染、stale edit、误替换、危险 shell、session resume，而不是只讲“能自动写代码”。

第三，看你是否理解模型协议层。高分回答要能讲 Anthropic streaming 的 `content_block_start/delta/stop`、partial_json 组装、OpenAI-compatible SSE delta 的差异，以及为什么要统一成 `ModelResult`。

第四，看你是否有验证意识。高分回答要说离线测试覆盖 streaming tool assembly、MCP stdio discovery/call、文件安全、上下文压缩、session 恢复。Agent 项目如果没有可复现测试，很容易被认为只是 demo。

### 三、最加分的技术点

最加分的点是 `ReAct Agent Loop`。你要把它讲成 Agent Runtime 的主线：LLM 不是只输出答案，而是根据 messages 决策是否调用工具，工具执行后返回 Observation，模型再基于新状态继续决策。这个闭环是所有后续能力的基础。

第二个加分点是上下文压缩。不要说成“把长文本截断”，而要说成四层治理：大工具结果落盘保留可恢复性，旧结果 snip 降低噪声，最近结果保留保证下一步决策质量，LLM summary 在高压情况下压缩任务状态。这里体现的是 token 成本、信息完整性和模型注意力之间的取舍。

第三个加分点是文件安全。`read-before-edit` 解决模型没观察真实文件就编辑的问题，`mtime` 解决外部修改导致的 stale edit，`old_text` 唯一性解决误替换，`unified diff` 解决修改可审计。这个点非常像真实 Coding Agent 的工程问题，面试官会喜欢。

第四个加分点是 MCP。MCP 不要讲成“又接了一个工具”，而要讲成工具生态协议化：外部 server 通过 JSON-RPC over stdio 提供 `tools/list` 和 `tools/call`，Agent 将其映射为统一 tool schema，再接入同一个 Agent Loop。

第五个加分点是 Sub-agent 和 Skills。Sub-agent 解决任务隔离和主上下文污染，Skills 解决领域流程按需注入。两者一个偏执行上下文隔离，一个偏知识加载机制，不要混着讲。

### 四、最容易被追问打穿的地方

第一，不要把项目说成完整生产级 Claude Code。更准确的说法是“核心机制复现与工程化增强”。如果面试官问和 Claude Code 差距，要主动说差在权限系统、安全沙箱、真实并发调度、终端 UI、长期记忆、评测体系和工具生态规模。

第二，不要说 streaming 已经完整实现生产级早期执行。当前更稳的说法是：项目实现了 streaming tool call 的协议组装，Anthropic 侧可以在 `content_block_stop` 后得到完整工具块，但 Agent 层主要还是响应收束后执行；生产化可以扩展为只读工具的异步提前执行。

第三，不要把危险 shell 检测说成安全沙箱。当前是学习/面试级防护，可以拦截常见危险命令；生产级需要容器隔离、文件系统权限、命令 allowlist、审计日志和人工审批。

第四，不要编压缩收益数据。如果被问 token 降低多少，可以回答：当前重点是机制正确性和可恢复性验证，后续会用 coding benchmark 统计压缩前后 token、工具调用次数、任务成功率和 resume 成功率。

第五，不要混淆 Skills 和 MCP。Skills 是本地领域流程/知识注入，MCP 是外部工具发现和调用协议。一个回答“应该怎么做”，一个回答“能调用什么外部能力”。

### 五、面试官可能的追问链路

追问链路 1：你说 ReAct Agent Loop，具体 messages 怎么变化？

推荐回答：用户任务进入 messages 后，模型返回 assistant message，其中包含 text 和 tool_use；Agent 执行 tool_use 后，把 tool_result 作为 Observation 回写到下一轮 messages；模型再基于这个 Observation 继续决策。这个循环直到模型不再返回 tool call，或者达到 max_turns / budget 限制。

追问链路 2：为什么上下文压缩不能直接删除旧消息？

推荐回答：直接删除会破坏任务状态，也可能让模型丢失关键约束。我的策略是优先保留可恢复性：大结果落盘，messages 里保留路径、长度和预览；旧结果只 snip，不完全抹掉；最近 Observation 保持完整；最后才用 LLM summary 汇总目标、约束、读写文件、错误和未完成事项。

追问链路 3：自动编辑代码最核心的风险是什么？

推荐回答：核心风险是模型基于过期或不完整上下文修改文件。read-before-edit 保证编辑前观察真实文件，mtime 防止读完后文件被外部改动，old_text 唯一性防止替换到错误位置，unified diff 让修改可审计。这几个机制组合起来，降低 stale edit、误替换和不可见修改的风险。

追问链路 4：MCP 为什么要做命名前缀？

推荐回答：因为不同 MCP server 或内置工具可能有同名工具，比如 search、read_file。用 `mcp__server__tool` 可以把工具列表展平成模型可见的 schema，同时保留 server 和 tool 的路由信息，调用时再解析前缀分发到对应 MCP connection。

追问链路 5：这个项目怎么证明不是纸面设计？

推荐回答：我写了离线测试，不依赖真实 API key，覆盖 Anthropic streaming tool assembly、MCP stdio discovery/call、read-before-edit、edit diff、大结果落盘压缩、session save/load 和危险 shell segment 检测。这样关键工程逻辑可以稳定复现，不靠现场模型随机发挥。

### 六、STAR 口径总回答

Situation：我做这个项目的背景是想系统理解 Claude Code 类 Coding Agent 为什么能执行长代码任务，而不是只停留在 LLM API 调用层。

Task：我的目标是复现一个轻量级 Agent Runtime，让模型能在 ReAct Loop 中自主决策工具调用，并补齐长任务中必须面对的上下文、安全、工具扩展和会话恢复问题。

Action：我按 v0-v6 渐进实现：先完成最小 Agent Loop，再加入文件工具、Todo、Sub-agent、Skills；最终在 v5/v6 中实现 read-before-edit、mtime、old_text 唯一性、unified diff、4 层上下文压缩、MCP JSON-RPC stdio、Anthropic/OpenAI-compatible streaming 适配、session resume 和 cost budget。

Result：最终项目形成了可运行、可测试、可讲解的 Coding Agent 核心机制复现。离线测试覆盖 streaming tool assembly、MCP 调用、文件安全、上下文压缩和 session 恢复。这个结果让我能从 Agent Runtime 的角度解释模型决策、工具执行、状态管理和工程安全，而不是只讲“调用大模型生成代码”。

### 七、简历描述应该怎么对齐

简历第一句要负责定性：这是 Coding Agent Runtime，不是普通 Chatbot。

推荐写法：

> 基于 Python 复现并扩展 Claude Code 类 Coding Agent 核心机制，构建“模型决策 - Tool Calling - 工具执行 - Observation 回写”的 ReAct Agent Loop，支持文件读写、命令执行、代码搜索、Todo 规划、Sub-agent、Skills 和 MCP 工具接入。

第二句要负责讲长任务能力：

> 针对 Coding Agent 长任务中的上下文膨胀和工具结果污染，设计 4 层上下文压缩策略，包括大结果落盘、旧结果 snip、最近 Observation 保留和 LLM 摘要压缩，并支持 session resume 保存 messages、token usage、权限状态和文件读取状态。

第三句要负责讲安全能力：

> 针对自动代码编辑可靠性，实现 read-before-edit、mtime 追踪、old_text 唯一性检查、unified diff 和危险 shell 检测，降低 stale edit、误替换和高风险命令执行带来的工程风险。

第四句要负责讲协议和验证：

> 实现 MCP JSON-RPC stdio 工具发现/调用机制，并适配 Anthropic / OpenAI-compatible 双后端 streaming，统一抽象为内部 ModelResult；编写离线测试覆盖 streaming tool assembly、MCP 调用、文件安全、上下文压缩和 session 恢复。

### 八、最终建议

这个项目在面试里应该主打“Agent 工程深度”，不要主打“功能多”。最强叙事顺序是：Agent Loop 是主线，上下文压缩解决长任务，文件安全解决自动编辑风险，Sub-agent/Skills 解决任务隔离和知识注入，MCP/ModelResult 解决工具生态和模型后端抽象，离线测试证明不是纸面方案。

一句话收束：

> 我这个项目的价值不是复刻 Claude Code 的全部产品能力，而是把 Coding Agent 从模型调用到工具执行、上下文治理、安全编辑和工具生态扩展的核心链路做成了一个可运行、可验证、可解释的最小系统。
